...
export default function AttachmentTable({ ofacId }) {
  const linkType = 'OFAC';
  ...
  const handleSearch = async (noNotif) => {
    try {
      const { attachedFilesList } = await listFile(ofacId, linkType);
  ...
  const handleAttachFile = async (e) => {
    const newRows = await attachFiles(ofacId, linkType, e.target.files);
