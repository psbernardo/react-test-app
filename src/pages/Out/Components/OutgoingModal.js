...
export default function OutgoingModal({
  onClose: handleClose,
  open: isOpen,
  value,
}) {
  ...
  const handleDownload = async (modalData) => {
    try {
      ...
      if (!valid) {
        ...
        const resp = await downloadOutgoing(modalData);
        if (resp.array.length != 0) {
          await downloadFile(resp.array[0][0]).then(async (items) => {
            if (resp.array[0].length > 1) {
              await downloadFile(resp.array[0][1]);
            }
          });

          notifySuccess('File Downloaded');
        } else {
          notifyWarning('No File to Download.');
        }
      }
    } catch (error) {
      console.error(error);
  ...
  //       return;
  //     } else {
  //       const resp = await transferOutgoing(modalData);
