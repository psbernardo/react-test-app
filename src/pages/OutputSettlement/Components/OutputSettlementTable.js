...
import { listOutputSettlement } from '../../../services/AcatsOutputSettlementService';
...
export default function OutputSettlementTable({ params }) {
  ...
  const columns = [
    ...
    {
      name: 'originalReceiverCustomerSsNumberTaxIdPrimary',
      label: 'Original Receiver Customer  Tax ID Primary',
      options: {
        display: false,
      },
    },
    ...
    {
      name: 'tranferTypeReject',
      label: 'Tranfer Type Reject',
      options: {
        display: false,
      },
    },
  ...
  const getCsvData = async () => {
    const data = await listOutputSettlement(searchData);
    return data.outputSettlementsList;
  };
  ...
  const handleSearch = async () => {
    try {
      ...
      const data = await listOutputSettlement(searchData);
      setRows(data.outputSettlementsList);

      notifyInfo(data.outputSettlementsList.length + ' search results.');
    } catch (error) {
