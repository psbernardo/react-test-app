...
import { listTradeParticipationDetails } from '../../../services/TradeParticipationService';
import { Close as CloseIcon } from '@material-ui/icons';
import Table, { columnType } from 'components/Table/Table';

export default function TradeParticipationModal({
  onClose: handleClose,
  open: isOpen,
  value,
}) {
  ...
  useEffect(
    () => {
      async function init() {
        console.log(value);
        const data = await listTradeParticipationDetails(value);
        console.log(data);
      ...
      <Fade in={isOpen}>
        <div className={classes.tableModalFull}>
          <Box component="div" mt={2}>
            <Table
              title="Trade Participation Details"
              data={rows}
              columns={columns}
              options={options}
            />
